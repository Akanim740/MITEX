...
function otpEmail(user, otp) {
  return {
    ...
    html: emailWrap(`<p style="color:#e5e7eb;font-size:15px;line-height:1.6;">Hi ${esc(user.name)},</p><p style="color:#e5e7eb;font-size:15px;line-height:1.6;">Use the code below to verify your email address and secure your account:</p><div style="background:#1f2937;border-radius:12px;padding:22px;margin:18px 0;text-align:center;"><span style="font-family:'Courier New',monospace;font-size:38px;font-weight:800;letter-spacing:10px;color:#fbbf24;">${esc(otp)}</span></div><p style="color:#9ca3af;font-size:13px;margin-top:14px;">This code expires in 10 minutes. If you didn't create this account, you can safely ignore this email.</p>`),
  };
}
...
function salaryEmail(staff, payment) {
  return {
    ...
    html: emailWrap(`<p style="color:#e5e7eb;font-size:15px;line-height:1.6;">Hi ${esc(staff.name)},</p><p style="color:#e5e7eb;font-size:15px;line-height:1.6;">Your salary has been recorded as paid.</p><div style="background:#1f2937;border-radius:8px;padding:16px 20px;margin:16px 0;"><table style="width:100%;font-size:14px;color:#d1d5db;"><tr><td style="padding:4px 0;">Period</td><td style="padding:4px 0;text-align:right;color:#fff;">${esc(payment.period)}</td></tr><tr><td style="padding:4px 0;">Amount</td><td style="padding:4px 0;text-align:right;color:#fbbf24;font-weight:700;">${naira(payment.amount)}</td></tr>${payment.bonus ? `<tr><td style="padding:4px 0;">Bonus</td><td style="padding:4px 0;text-align:right;color:#10b981;">${naira(payment.bonus)}</td></tr>` : ""}${payment.note ? `<tr><td style="padding:4px 0;">Note</td><td style="padding:4px 0;text-align:right;color:#fff;">${esc(payment.note)}</td></tr>` : ""}</table></div><p style="color:#9ca3af;font-size:13px;">Keep this email as your record.</p>`),
  };
}
...
function deliveryEmail(buyer, order) {
  return {
    ...
    html: emailWrap(`<p style="color:#e5e7eb;font-size:15px;line-height:1.6;">Hi ${esc(buyer.name)},</p><p style="color:#10b981;font-size:16px;font-weight:700;">Your website is ready!</p><p style="color:#e5e7eb;font-size:15px;line-height:1.6;">The website you purchased (<strong style="color:#fff;">${esc(order.title)}</strong>) is now available for download.</p>${emailBtn(APP_URL + "/account.html", "Download Now")}<p style="color:#9ca3af;font-size:13px;margin-top:20px;">If you need help, WhatsApp +234 701 163 3770</p>`),
  };
}
...
function buyerWaitingEmail(worker, listing) {
  return {
    ...
    html: emailWrap(`<p style="color:#e5e7eb;font-size:15px;line-height:1.6;">Hi ${esc(worker.name)},</p><p style="color:#e5e7eb;font-size:15px;line-height:1.6;">A buyer is ready to purchase <strong style="color:#fff;">"${esc(listing.title)}"</strong> and is waiting for the delivery link.</p><p style="color:#e5e7eb;font-size:15px;line-height:1.6;">Add the delivery link in your <a href="${APP_URL}/worker.html" style="color:#fbbf24;">staff dashboard</a> as soon as you're done - the buyer will be notified automatically.</p>`),
  };
}
...
function listingReadyEmail(buyer, listing) {
  return {
    ...
    html: emailWrap(`<p style="color:#e5e7eb;font-size:15px;line-height:1.6;">Hi ${esc(buyer.name)},</p><p style="color:#10b981;font-size:16px;font-weight:700;">Good news!</p><p style="color:#e5e7eb;font-size:15px;line-height:1.6;">The website you were waiting for (<strong style="color:#fff;">"${esc(listing.title)}"</strong>) is now ready to buy.</p>${emailBtn(APP_URL + "/marketplace.html", "Buy Now")}`),
  };
}
